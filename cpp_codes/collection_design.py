...
class CompanyUserCollection:
    def __init__(self):
        """회사 회원 콜렉션
        전혀 파이썬 스럽지 않지만, C++ 에 가깝게 배열 형태로 사용
        """
        self.users: List[CompanyUser | None] = [None] * 100
        self.total_users = 0

    def _find_user_index_by_id(self, id_: str) -> int:
        """private 함수, 유저 ID를 찾아 배열 내 Index 반환
    ...
    def is_user_exists(self, id_: str) -> bool:
        """유저 존재 여부 확인. 존재시 참, 없을시 거짓"""

        return self._find_user_index_by_id(id_) != -1

    def add_user(self, user: CompanyUser) -> None:
    ...
    def get_user_by_id(self, id_: str) -> CompanyUser:
        """로그인 ID로 유저 정보 획득
...
class CommonUserCollection:
    def __init__(self):
        """일반 회원 콜렉션"""

        self.users: List[CommonUser | None] = [None] * 100
        self.total_users = 0
    ...
    def add_user(self, user: CommonUser) -> None:
        """새 유져 추가. C++ 구현시 self.total_users++ 로 후위 연산자 사용할 것
    ...
    def get_user_by_id(self, id_: str) -> CommonUser:
        """로그인 ID 로 유저 정보 획득."""
...
class UserCollection:
    def __init__(self, company_collection, common_collection):
        self.company_collection: CompanyUserCollection = company_collection
        self.common_collection: CommonUserCollection = common_collection
    ...
    def delete_user_by_id(self, id_: str) -> None:
        ...
        if self.company_collection.is_user_exists(id_):
