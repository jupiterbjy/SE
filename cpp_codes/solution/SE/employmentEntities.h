...
public:
    ...
    void addEmployment(const string& companyName, const string& deadline, const string& work){
        Employment new_employment = Employment(companyName, deadline, work);
        cout << companyName << endl;
        employmentList[numEmployments] = &new_employment;
        cout << employmentList[0]->getCompanyName() << endl;
        numEmployments++;
    }
...
public:
    void addEmployment(){
        ...
        cout << temp << endl;
        employmentAddUI->showAddEmployment();
        return;
	}
};
...
public:
    void showEmploymentListF(string companyName, string work, string deadline){
...
public:
    void showEmploymentInfo(){
        ...
        for(index=0;index<total;index++){
            ...
            cout << temp << endl;
            showEmploymentListUI->showEmploymentListF(employmentCollection->getCompanyName(index), employmentCollection->getWork(index), employmentCollection->getDeadline(index));;
            index++;
			//  if 사용자랑 == 현재 유저가 같으면 
